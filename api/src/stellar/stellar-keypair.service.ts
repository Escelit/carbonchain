...
export class StellarKeypairService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (secret && secret !== 'S...') {
      try {
        ...
        this.logger.log(
          `Admin keypair loaded for public key: ${this.adminKeypair.publicKey()}`,
        );
      } catch (error: unknown) {
        this.logger.error(
          'Failed to load admin keypair from SECRET_KEY',
          (error as Error).stack,
        );
      }
    } else {
      this.logger.warn(
        'ADMIN_SECRET_KEY not set or using placeholder. Admin functions will be unavailable.',
      );
    }
  }
  ...
  getAdminKeypair(): Keypair {
    if (!this.adminKeypair) {
      throw new Error(
        'Admin keypair is not initialized. Check ADMIN_SECRET_KEY in .env',
      );
    }
